...
int main(int argc,char**argv){
  ...
  if (exo == 0) {
    int coups = sequence_aleatoire_rec(M, G, dim, nbcl, aff);
    printf("Exo 0 – Random recursive: %d coups\n", coups);
}

else if (exo == 1) {
    int coups = sequence_alea_rapide(M, G, dim, nbcl, aff);
    printf("Exo 1 – Random Zsg method: %d coups\n", coups);
}

else if (exo == 2) {
    int coups = exo_max_bordure(M, G, dim, nbcl, aff);
    printf("Exo 2 – Max border: %d coups\n", coups);
}
else if (exo == 3) {
        
        int nb_iterations = strategie_max_bordure_zone(M, G, dim, nbcl, aff);
        printf("Exo 3 -Max border zone: %d iterations\n", nb_iterations);
    }
else if (exo == 4) {
        ...
        */ 
       int nb_iterations = sequence_greedy_astar(M, dim, nbcl, G, aff);
        printf("Personal strategy : %d iterations\n", nb_iterations);
